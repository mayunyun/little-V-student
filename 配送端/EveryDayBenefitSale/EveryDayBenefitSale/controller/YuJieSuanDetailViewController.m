//
//  YuJieSuanDetailViewController.m
//  EveryDayBenefitSale
//
...
#import "YujiesuanModel.h"
#import "YujiesuanTableViewCell.h"

@interface YuJieSuanDetailViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    NSInteger _page;
    UITableView* _tbView;
...
@implementation YuJieSuanDetailViewController
...
- (void)viewDidLoad {
    ...
    self.title = @"结算查询";
    [self creatUI];
    [self searchalldaqudetailRuestData];
    //进度HUD
...
- (void)sureBtnClick:(UIButton*)sender
{
    if ([_startlabel.text isEqualToString:@"请选择"]&&[_endLabel.text isEqualToString:@"请选择"]&&[_cashnoTextField.text isEqualToString:@""]) {
        ...
        [self searchalldaqudetailRuestData1];
    }
}
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    YujiesuanTableViewCell* detailCell = [tableView dequeueReusableCellWithIdentifier:@"YujiesuanTableViewCellID"];
    if (!detailCell) {
        detailCell = [[[NSBundle mainBundle]loadNibNamed:@"YujiesuanTableViewCell" owner:self options:nil]firstObject];
    }
    if (tableView == _tbView) {
        if (!IsEmptyValue(_dataArray)) {
            YujiesuanModel* model = _dataArray[indexPath.row];
            detailCell.custnameLabel.text = [NSString stringWithFormat:@"%@",model.custname];
            detailCell.danjuhaoLabel.text = [NSString stringWithFormat:@"%@",model.danjuhao];
            detailCell.createtimeLabel.text = [NSString stringWithFormat:@"%@",model.createtime];
            detailCell.qichuLabel.text = [NSString stringWithFormat:@"%.2f",[model.qichu doubleValue]];
            detailCell.jiecunLabel.text = [NSString stringWithFormat:@"%.2f",[model.jiecun doubleValue]];
            detailCell.inmoneyLabel.text = [NSString stringWithFormat:@"%.2f",[model.inmoney doubleValue]];
            detailCell.outmoneyLabel.text = [NSString stringWithFormat:@"%.2f",[model.outmoney doubleValue]];
        }
...
- (void)searchalldaqudetailRuestData
{
    /*
     /report/searchalldaqudetail1.do
     data:{"provinceid":"","cityid":"","areaid":"","serviceid":"","villageid":"","custidEQ":"153","danjuhaoEQ":"dd201611160001","createtimeGE":"2016-11-16","createtimeLE":"2016-11-16","settlement_id":226}
     */
    if (_page == 1) {
        [_dataArray removeAllObjects];
    }
    NSString* urlstr = [NSString stringWithFormat:@"%@%@",ROOT_Path,@"/report/searchalldaqudetail1.do"];
    NSString* datastr = [NSString stringWithFormat:@"{\"settlement_id\":\"%@\"}",self.Id];
    NSDictionary* parmas = @{@"mobile":@"true",@"data":datastr,@"page":[NSString stringWithFormat:@"%li",(long)_page],@"rows":@"20"};
    [DataPost requestAFWithUrl:urlstr params:parmas finishDidBlock:^(id result) {
        [_hud hide:YES];
        NSString* str = [[NSString alloc]initWithData:result encoding:NSUTF8StringEncoding];
        NSLog(@"/report/searchalldaqudetail1.do%@",str);
            ...
            if (!IsEmptyValue(dict)) {
                ...
                if (!IsEmptyValue(array)) {
                    for (int i = 0; i < array.count; i++) {
                        YujiesuanModel* model = [[YujiesuanModel alloc]init];
...
- (void)searchalldaqudetailRuestData1
{
    /*
     /report/searchalldaqudetail1.do
     mobile:true
     ...
     rows:
     data:{
     "danjuhaoEQ":"dd201611160001",
     "createtimeGE":"2016-11-16",
     "createtimeLE":"2016-11-16",
     "settlement_id":226
     }
     */
    NSString* startTime;
    ...
    NSString* urlstr = [NSString stringWithFormat:@"%@%@",ROOT_Path,@"/report/searchalldaqudetail1.do"];
    NSString* datastr = [NSString stringWithFormat:@"{\"danjuhaoEQ\":\"%@\",\"createtimeGE\":\"%@\",\"createtimeLE\":\"%@\",\"settlement_id\":\"%@\"}",_cashnoTextField.text,startTime,endTime,self.Id];
    NSDictionary* parmas = @{@"mobile":@"true",@"data":datastr,@"page":@"1",@"rows":@"1000000"};
    [DataPost requestAFWithUrl:urlstr params:parmas finishDidBlock:^(id result) {
         [_hud hide:YES];
        NSString* str = [[NSString alloc]initWithData:result encoding:NSUTF8StringEncoding];
        NSLog(@"/report/searchalldaqudetail1.do%@",str);
