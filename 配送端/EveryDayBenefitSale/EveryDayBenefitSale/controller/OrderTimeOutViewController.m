        ...
        if (!IsEmptyValue(_dataArray)) {
            {
                ...
                cell.nameLabel.text = [NSString stringWithFormat:@"%@",model.sendername];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView == _tbView) {
        if (!IsEmptyValue(_dataArray)) {
            ...
            vc.orderNo = [NSString stringWithFormat:@"%@",model.orderno];
...
- (void)dataRequest
{
    ...
    NSString* datastr = [NSString stringWithFormat:@"{\"senderid\":\"%@\",\"page\":\"%li\",\"rows\":\"20\"}",userid,(long)_page];
    ...
    [DataPost requestAFWithUrl:urlstr params:params finishDidBlock:^(id result) {
        ...
        NSLog(@"/order/OrderTimeOut.do%@",str);
