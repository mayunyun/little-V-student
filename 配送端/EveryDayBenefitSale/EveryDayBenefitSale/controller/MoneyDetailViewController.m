...
//    [firstCellView addSubview:_monthMoneyLabel];
    _monthMoneyBtn = [UIButton buttonWithType:UIButtonTypeSystem];
    _monthMoneyBtn.frame = CGRectMake(_monthLabel.right, 0, mScreenWidth - 60 - 30, _monthLabel.height);
    [_monthMoneyBtn setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    _monthMoneyBtn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
    [firstCellView addSubview:_monthMoneyBtn];
    [_monthMoneyBtn addTarget:self action:@selector(moneyBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    
    UIButton* riliBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    riliBtn.frame = CGRectMake(_monthMoneyBtn.right , 10, 30, 30);
...
//    _monthMoneyLabel.text = @"￥100.00";
   [_monthMoneyBtn setTitle:@"￥100.00" forState:UIControlStateNormal];
