...
- (void)creatSubViews
{
    ...
    UILabel* userleftLabel = [[UILabel alloc]initWithFrame:CGRectMake(15, userLabel.top, 40, 49)];
    ...
    UILabel* pwdleftLabel = [[UILabel alloc]initWithFrame:CGRectMake(15, pwdLabel.top, 40, 49)];
    ...
    UILabel* autologinLabel = [[UILabel alloc]initWithFrame:CGRectMake(_autologin.right+5, jizhuyonghu.top, 100, 30)];
...
- (void)requestLogin:(UIButton*)sender {
    ...
    [DataPost requestAFWithUrl:urlStr params:param finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
        sender.enabled = YES;
        ...
        if ([str rangeOfString:@"msg"].location != NSNotFound) {
            ...
            if (array!=0){
                ...
                }else if ([model.custtypeid integerValue] != 2){
                    [self showAlert:@"账号非分销端账号"];
                }else{
...
- (void)versionRequest{
    ...
    NSDictionary *parameters = @{@"action":@"getVersionInfo",@"project":[NSString stringWithFormat:@"%@",@"applexiaoweiSJ"]};
