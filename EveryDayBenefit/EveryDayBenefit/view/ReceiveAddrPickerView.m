...
#import "ReceiveAddrPickerView.h"
...
@implementation ReceiveAddrPickerView
...
- (void)ProvinceRequest
{
  ...
  */
    ...
    NSString* urlStr = [NSString stringWithFormat:@"%@/areamanage/loadprovinceadd.do?mobile=true",ROOT_Path];
 ...
 */
    [city removeAllObjects];
    NSString* urlStr = [NSString stringWithFormat:@"%@/areamanage/loadcityadd.do?mobile=true&&provinceid=%@",ROOT_Path,provinceID];
    [HTNetWorking postWithUrl:urlStr refreshCache:YES params:nil success:^(id result) {
        NSString* str = [[NSString alloc]initWithData:result encoding:NSUTF8StringEncoding];
        if ([str rangeOfString:@"sessionoutofdate"].location != NSNotFound) {
 ...
 */
    [district removeAllObjects];
    NSString* urlStr = [NSString stringWithFormat:@"%@/areamanage/loadareaadd.do?mobile=true&&cityid=%@",ROOT_Path,cityID];
    [HTNetWorking postWithUrl:urlStr refreshCache:YES params:nil success:^(id result) {
        NSString* str = [[NSString alloc]initWithData:result encoding:NSUTF8StringEncoding];
        if ([str rangeOfString:@"sessionoutofdate"].location != NSNotFound) {
...
{
/*
 /address/addressVillage.do?mobile=true&&centerid
 */
    [village removeAllObjects];
