//
//  OrderManageProViewController.m
//  EveryDayBenefit
//
...
#define cellHight 80
@interface OrderManageProViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    UITableView* _tbView;
...
@implementation OrderManageProViewController
...
- (void)creatUI
{
    ...
    _tbView = [[UITableView alloc]initWithFrame:CGRectMake(0, 30, mScreenWidth, mScreenHeight - 64 - 30)];
    _tbView.backgroundColor = [UIColor whiteColor];
    _tbView.delegate = self;
    _tbView.dataSource = self;
    _tbView.rowHeight = 180;
...
{
    if (!_myAleartView) {
        _myAleartView = [[UIView alloc]initWithFrame:CGRectMake(mScreenWidth - 80, 0, 80, 45*6)];
    ...
    if (tableView == _tbView) {
        if (!IsEmptyValue(_dataArray)) {
            ...
            if ([model.isgolds integerValue] == 1) {
                cell.priceLabel.text = [NSString stringWithFormat:@"合计：%@金币",model.ordermoney];//@"总金额**元";
            }else{
                
                cell.priceLabel.text = [NSString stringWithFormat:@"合计：%.2f元",[model.ordermoney floatValue]];//@"总金额**元";
            }
            NSString* str;
            NSString* selectStr;
            switch ([model.orderstatus intValue]) {
                case -1:
                    str = [NSString stringWithFormat:@"共%@件商品",model.ordercount];// | 已取消订单
                    selectStr = @"已取消";
                    cell.delBtn.hidden = NO;
                    break;
                case 0:
                    str = [NSString stringWithFormat:@"共%@件商品",model.ordercount];// | 等待付款
                    selectStr = @"等待付款";
                    break;
                case 1:
                    str = [NSString stringWithFormat:@"共%@件商品",model.ordercount];// | 等待收货
                    selectStr = @"等待收货";
                    break;
                case 2:
                {
                    if (IsEmptyValue(model.exiteflag)) {
                        str = [NSString stringWithFormat:@"共%@件商品",model.ordercount];// | 已完成
                        selectStr = @"已完成";
                    }else if([model.exiteflag intValue] == 0){
                        str = [NSString stringWithFormat:@"共%@件商品",model.ordercount];// | 等待退货
                        selectStr = @"等待退货";
                    }else if ([model.exiteflag intValue] == 1){
                        str = [NSString stringWithFormat:@"共%@件商品",model.ordercount];// | 退货完成
                        selectStr = @"退货完成";
                    }else if ([model.exiteflag intValue] == -1){
                        str = [NSString stringWithFormat:@"共%@件商品",model.ordercount];// | 退货拒绝
                        selectStr = @"退货拒绝";
                    }
                }
                    ...
                    break;
                case 3:
                    str = [NSString stringWithFormat:@"共%@件商品",model.ordercount];// | 等待评价
                    selectStr = @"等待评价";
                    break;
                    //                case 4:
                    //
                    ...
                    //                    break;
                case 6:
                    str = [NSString stringWithFormat:@"共%@件商品",model.ordercount];// | 待发货
                    selectStr = @"待发货";
                    break;
                default:
                    break;
            ...
            cell.dateString = model.createtime;
        }
        [cell setNeedsLayout];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
    }else{
        NSArray* array = @[@"待付款",@"待发货",@"待收货",@"待评价",@"已完成",@"已取消"];
