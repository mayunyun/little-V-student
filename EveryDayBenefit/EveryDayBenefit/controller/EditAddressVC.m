...
@interface EditAddressVC ()<UITextFieldDelegate,AccountDelegate>
{
    UISwitch* _setSwitch;
...
- (void)viewDidLoad {
    ...
    _addModel.provincename = [self convertNull:_addModel.provincename];
    _addModel.cityname = [self convertNull:_addModel.cityname];
    _addModel.areaname = [self convertNull:_addModel.areaname];
    _addModel.servincename = [self convertNull:_addModel.servincename];
    _addModel.linker = [self convertNull:_addModel.linker];
    _addModel.linkerid = [self convertNull:_addModel.linkerid];
//    NSLog(@"_addModel---%@,%@,%@,%@,%@",_addModel.provinceid,_addModel.cityid,_addModel.areaid,_addModel.serviceid,_addModel.linkerid);
    [self backBarTitleButtonItemTarget:self action:@selector(backClick:) text:@"地址管理"];
...
- (void)creatUI
{
    ...
    _shdetailLabel.text = [NSString stringWithFormat:@"%@    |    %@    |    %@",_addModel.provincename,_addModel.cityname,_addModel.areaname];
    ...
    [_curAccount setTitle:[NSString stringWithFormat:@"%@",_addModel.servincename] forState:UIControlStateNormal];
    _curAccountID = [NSString stringWithFormat:@"%@",_addModel.serviceid];
    // 显示框背景色
    ...
    [_curAccount1 setTitle:[NSString stringWithFormat:@"%@",_addModel.doornumbername] forState:UIControlStateNormal];
    NSLog(@"这是个什么玩意%@",_addModel.doornumber);
    ...
    // 字体
    [_curAccount2 setTitle:[NSString stringWithFormat:@"%@",_addModel.comunity] forState:UIControlStateNormal];
    ...
    // 字体
    [_curAccount3 setTitle:[NSString stringWithFormat:@"%@",_addModel.roomnumber] forState:UIControlStateNormal];
    ...
    _detailTextField.text = [NSString stringWithFormat:@"%@",_addModel.address];
//    _detailTextField.placeholder = @"请输入详细地址";
    NSLog(@"小区名字是在玩意%@",_addModel.doornumbername);
    ...
    _nameTextField.text = [NSString stringWithFormat:@"%@",_addModel.addressname];
//    _nameTextField.placeholder = @"请输入姓名";
    ...
    [sureBtn setImage:[UIImage imageNamed:@"baocun1.png"] forState:UIControlStateNormal];
    [bgView addSubview:sureBtn];
    [sureBtn addTarget:self action:@selector(sureBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    _curAccountID = _addModel.doornumber;
    _curAccount1ID = _addModel.serviceid;
    _curAccount2ID = _addModel.comunityid;
    _curAccount3ID = _addModel.roomnumberid;
...
- (void)openAccountList {
    ...
    if (_accountList.isOpen) {
        ...
        _curAccount3ID = @"";
//        NSLog(@"-_listFrame,-----------%f,%f,%@",_listFrame.origin.y,_listFrame.size.height,_addModel.areaid);
        [_accountList.view setFrame:_listFrame];
        NSLog(@"我的viligeId是什么%@",_addModel.villageid);
        if (!IsEmptyValue(_addModel.villageid)) {
            [self ServiceCenterRequest:[NSString stringWithFormat:@"%@",_addModel.villageid]];
        }
    }
    else {
    ...
    if (_accountList1.isOpen) {
        ...
        if (!IsEmptyValue(_addModel.villageid)) {
            ...
            if (!IsEmptyValue(_curAccountID)) {
                [self VillageRequest:[NSString stringWithFormat:@"%@",_curAccountID] villageid:[NSString stringWithFormat:@"%@",_addModel.villageid]];
            }else{
                [self VillageRequest:[NSString stringWithFormat:@"%@",_curAccountID] villageid:[NSString stringWithFormat:@"%@",_addModel.villageid]];
            }
        }
...
{
    //    NSString *showMsg = [NSString stringWithFormat:@"省%@市%@区%@街道%@门牌%@小区%@楼号%@详细%@收货人%@默认地址%hhd",_addModel.province,_addModel.city,_addModel.area,_curAccountID,_curAccount1ID,_curAccount2ID,_curAccount3ID,_detailTextField.text,_nameTextField.text,_setSwitch.on];
    //    NSLog(@"地址选择器返回值%@",showMsg);
    
//    if (!IsEmptyValue(_curAccountID)&&!IsEmptyValue(_curAccount1ID)&&!IsEmptyValue(_curAccount2ID)&&!IsEmptyValue(_curAccount3ID)&&!IsEmptyValue(_detailTextField.text)) {
//
//    }else{
...
//    _addModel.provinceid = [self convertNull:_addModel.provinceid];
//    _addModel.cityid = [self convertNull:_addModel.cityid];
//    _addModel.areaid = [self convertNull:_addModel.areaid];
...
//    _addModel.linkerid = [self convertNull:_addModel.linkerid];
    ...
    NSString* urlStr = [NSString stringWithFormat:@"%@%@",ROOT_Path,@"/login/updatecutAddress.do"];
    NSMutableDictionary* parmas = [[NSMutableDictionary alloc]init];
    [parmas setObject:@"true" forKey:@"mobile"];
    NSString* datastr = [NSString stringWithFormat:@"{\"address\":\"%@\",\"serviceid\":\"%@\",\"doornumber\":\"%@\",\"xiaoqu\":\"%@\",\"louhao\":\"%@\",\"addressname\":\"%@\",\"addrid\":\"%@\"}",_detailTextField.text,_curAccountID,_curAccount1ID,_curAccount2ID,_curAccount3ID,_nameTextField.text,_addModel.Id];
    ...
    [HTNetWorking postWithUrl:urlStr refreshCache:YES params:parmas success:^(id result) {
        ...
        if ([str rangeOfString:@"sessionoutofdate"].location != NSNotFound) {
            ...
            [self showAlert:@"收货地址修改成功"];
            [self.navigationController popViewControllerAnimated:YES];
        }else{
            [self showAlert:@"收货地址修改失败"];
        }
        [hud hide:YES];
    } fail:^(NSError *error) {
        [hud hide:YES];
