...
- (instancetype)initWithStarSize:(CGSize)size space:(CGFloat)space numberOfStar:(NSInteger)number highlight:(UIColor*)coler{
    self = [super init];
    if (self) {
        _selectable = YES;
        ...
        for (int i = 0; i < number; i ++) {
            ...
            if (coler == [UIColor blueColor]) {
                starView.highlight = NO;
            }else{
                starView.highlight = YES;
            }
...
//    NSLog(@"index%ld",(long)index);//这里表示星星的个数
//    NSString* str = [NSString stringWithFormat:@"%li",index+1];
    //    通过block反向传值
    if (_transVaule) {
        _transVaule(index+1);
    }
